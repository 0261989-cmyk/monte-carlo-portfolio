...
def descargar_datos_historicos(tickers_yf: list, periodo: str = "3y") -> pd.DataFrame:
    ...
    Compatible con yfinance >= 0.2.x que devuelve MultiIndex en columnas.

    Args:
        tickers_yf: Lista de tickers del proyecto
        periodo: Período histórico ('1y', '2y', '3y', '5y')

    Returns:
        DataFrame con log-retornos diarios limpios, una columna por activo.
    """
    yf_map = {
        "SPY": "SPY",
        "EEM": "EEM",
    ...
    for ticker in tickers_yf:
        try:
            ...
            data = yf.download(
                ...
                progress=False,
                auto_adjust=True,
                multi_level_index=False,   # yfinance >= 0.2.31: evita MultiIndex
            )
            if data.empty:
                st.warning(f"Sin datos para {ticker} ({yf_ticker})")
                continue

            # Extraer columna Close independientemente del formato
            if isinstance(data.columns, pd.MultiIndex):
                # MultiIndex: ('Close', 'SPY') → aplanamos
                close_cols = [c for c in data.columns if c[0] == "Close"]
                if close_cols:
                    precios = data[close_cols[0]].dropna()
                else:
            ...
            # Necesitamos al menos 2 filas para calcular retornos
            if len(precios) < 2:
            ...
            # Aplanar a Serie 1D si por algún motivo es DataFrame
    ...
    if not retornos:
        st.error("No se pudieron descargar datos. Se usarán los parámetros manuales.")
        return pd.DataFrame()

    # Alinear por fecha (inner join) y eliminar NaN
    df = pd.concat(retornos, axis=1).dropna()
    df.columns = list(retornos.keys())
    return df
    ...
    if ejecutar:
        ...
        if usar_datos_reales:
            with st.spinner("Descargando datos históricos de Yahoo Finance..."):
                try:
                    retornos_df = descargar_datos_historicos(
                        tuple(TICKERS), periodo_historico
                    )
                    if retornos_df is not None and not retornos_df.empty:
                        ...
                        if actualizados:
                            st.success(f"Parametros actualizados con datos reales: {', '.join(actualizados)}")
                        else:
                            st.warning("No se actualizo ningun activo. Se usan parametros manuales.")
                    else:
                        st.warning("No se obtuvieron datos historicos. Se usan parametros manuales.")
                except Exception as e:
                    st.warning(f"Error al descargar datos historicos: {e}. Se usan parametros manuales.")
